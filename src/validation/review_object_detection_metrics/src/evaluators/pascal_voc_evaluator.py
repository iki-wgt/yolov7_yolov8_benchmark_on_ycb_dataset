...
from review_object_detection_metrics.src.utils.enumerators import (BBFormat, CoordinatesType,
...
def plot_precision_recall_curve(results,
    ...
    plt.legend(shadow=True,bbox_to_anchor=(1.05, 1.0), loc='upper left')
    plt.grid()
    
    if savePath is not None:
        plt.savefig(os.path.join(savePath, 'all_classes.png'),bbox_inches='tight', facecolor='#FFFFFF')
...
def plot_precision_recall_curves(results,
    ...
    for classId, result in results.items():
        ...
        plt.ylim([-0.1, 1.1])
        if savePath is not None:
            plt.savefig(os.path.join(savePath, classId + '.png'),facecolor='#FFFFFF')
        if showGraphic is True:
            plt.show()
            # plt.waitforbuttonpress()
            plt.pause(0.05)
